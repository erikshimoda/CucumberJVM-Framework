...
public class UrlBuilder {
	...
	public static void startAtHomePage() {
		// WebDriverHelper.getSharedDriver().navigate().to((basePath));
	}
	...
	public static URI getBasePathURI(String url) {
		return URI.create(url);
	}

	public static URL createApiUrl(String endpoint) {
		try {
			return new URL(apiUrl.getProtocol(), apiUrl.getHost(),
					apiUrl.getPort(), endpoint);
		} catch (MalformedURLException e) {
	...
	// try {
	// return new URL(basePath.getProtocol(), basePath.getHost(),
	// basePath.getPort(), path);
	// } catch (MalformedURLException e) {
	// throw new RuntimeException(e);
	// }
	// }
